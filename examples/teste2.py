...
def pagina_grafico_animado_sexo():
    st.title("📊 População na Força de Trabalho por Sexo e Faixa Etária (Animação)")
    ...
    if df.empty:
        st.warning("Não foi possível carregar os dados para gerar os gráficos animados.")
        return

    # Criar grupo combinado
    df["Grupo"] = df["sex"] + " - " + df["features"]

    # Gráfico 1: Barras verticais animadas
    fig_vertical = px.bar(
        df,
        x="Grupo",
        y="work_pop",
        ...
        range_y=[0, df["work_pop"].max() * 1.1],
        labels={"work_pop": "População na Força de Trabalho", "Grupo": "Grupo"},
        title="Evolução da Força de Trabalho por Sexo e Faixa Etária (Vertical)"
    )
    fig_vertical.update_layout(xaxis_tickangle=-45)

    st.plotly_chart(fig_vertical, use_container_width=True)

    # Divider entre os gráficos
    st.divider()

    # Gráfico 2: Barras horizontais animadas
    fig_horizontal = px.bar(
        ...
        labels={"work_pop": "População na Força de Trabalho", "Grupo": "Grupo"},
        title="Evolução da Força de Trabalho por Sexo e Faixa Etária (Horizontal)"
    )
